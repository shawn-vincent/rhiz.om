...
test.describe("Dev Mode Authentication Navigation", () => {
	test("can navigate to @intraliminal space using dev mode auth", async ({
		page,
	}) => {
		...
		if (url.includes("/auth/signin")) {
			...
			const devLoginButton = page.locator(
				'button:has-text("🚀 Dev Login as @test-user-being")',
			);
		...
		console.log(
			"✅ Successfully navigated to @intraliminal space page with dev mode auth",
		);
	});

	test("dev mode signin page shows both dev and normal auth options", async ({
		page,
	}) => {
		...
		await expect(
			page.locator("text=Only available in development mode"),
		).toBeVisible();

		console.log("✅ Dev mode signin page shows both authentication options");
	});

	test("can complete full auth flow and interact with @intraliminal space", async ({
		page,
	}) => {
		...
		if (await menuButton.isVisible()) {
			...
			if (isSignInVisible) {
				console.log(
					"Sign in link still visible - authentication may have failed",
				);
		...
		console.log(
			"✅ Successfully completed full auth flow and verified authenticated state",
		);
	});
});
